...
/**
 * 필요 기능들
 * 채팅방 목록 조회 (닉네임 시간 마지막 내용)
 * 채팅방 세부 조회
 * 메시지 조회 -- 세부조화랑 갇을듯?
 * 읽음처리- 이건 세부 조회할때 처리하면 될듯 xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
 ...
 */
     ...
     */
    @GetMapping("/rooms")
    public ResponseEntity<List<ChatRoomDto>> getChatRooms(@AuthenticationPrincipal CustomUserDetails user, @RequestParam Enum.ChatType type) {
    ...
    public ResponseEntity<ChatEnterRes> getChatRoom(@AuthenticationPrincipal CustomUserDetails user, @PathVariable("roomId") Long roomId) {
     ...
     */
    @PostMapping("/rooms")
    public ResponseEntity<ChatEnterRes> createChatRoom(@AuthenticationPrincipal CustomUserDetails user, @RequestBody ChatRoomReq request) {
    ...
    public ResponseEntity<?> deleteRoom(@AuthenticationPrincipal CustomUserDetails user, @PathVariable("roomId") Long roomId) {
    ...
    public ResponseEntity<Void> updateTradeStatus(@AuthenticationPrincipal CustomUserDetails user, @PathVariable Long roomId, @RequestBody TradeStatusUpdateReq request) { // 상태 변경을 위한 DTO
