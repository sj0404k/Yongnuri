    ...
    public ChatEnterRes createChatRoom(CustomUserDetails user, ChatRoomReq request) {
        ...
        if (existing.isPresent()) {
            ...
            switch (request.getType()) {
                case LOST_ITEM -> {
                    LostItem lostItem = lostItemRepository.findById(request.getFromUserId())
                            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "분실물 게시글을 찾을 수 없습니다."));
                    targetUser = lostItem.getUser();
                }
                case USED_ITEM -> {
                    UsedItem usedItem = usedItemRepository.findById(request.getFromUserId())
                ...
                case GROUP_BUY -> {
                    GroupBuy groupBuy = groupBuyRepository.findById(request.getFromUserId())
